...
- [x] Task 3 — GitHub Actions CI pipeline (AC3)
  ...
  - [x] Open a test PR and confirm status checks appear (manual — verified via Railway deploy)

- [x] Task 4 — Railway deployment setup (AC4)
  - [x] Create Railway project and connect GitHub repo
  - [x] Enable auto-deploy from `main` branch
  - [x] Provision Railway PostgreSQL plugin
  - [x] Verify app is accessible at Railway HTTPS URL after first deploy

- [x] Task 5 — Configure Railway environment variables (AC5)
  - [x] Set `DATABASE_URL` (from Railway PostgreSQL plugin — copy from Railway dashboard)
  - [x] Set `SESSION_SECRET` (generate a strong random string, e.g. `openssl rand -hex 32`)
  - [x] Set `ADMIN_PASSWORD_HASH` (bcrypt hash of initial admin password — see Dev Notes)
...
- `drizzle.config.ts` had TS error (url: string | undefined) — fixed with explicit guard (`if (!process.env.DATABASE_URL) throw new Error(...)`), NOT a non-null assertion (`!`).
...
- `drizzle.config.ts` — fixed TS error (DATABASE_URL explicit guard, not non-null assertion)
...
- 2026-03-07: Code review fixes — H1: `src/styles.css` now imports `./styles/globals.css` (Campaign TOW palette tokens and fonts now loaded); H2: duplicate placeholder test `tests/integration/placeholder.test.ts` removed (kept `src/lib/placeholder.test.ts`). 33 tests pass.
- 2026-03-08: Code review cleanup — M1: `src/styles.css` stripped of scaffold design system (Fraunces/Manrope/sea-ink tokens removed, shadcn oklch vars kept); M2: scaffold demo cruft removed (demo routes, demo components, demo hooks, Header/Footer/ThemeToggle, todos schema); M3: `__root.tsx` rebranded (title="Campaign TOW", lang="fr", THEME_INIT_SCRIPT removed, scaffold Header/Footer removed); M6: `drizzle-kit` moved to devDependencies; L3: CI pnpm fixed to version 10. typecheck ✅ lint ✅ 33 tests ✅
